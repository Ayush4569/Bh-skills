...
export default function EditChallenge({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load modules list and challenge details
  useEffect(() => {
    const loadAll = async () => {
      try {
        const modulesRes = await fetch('/api/admin/labs/modules');
        const modulesData = await modulesRes.json();
        if (Array.isArray(modulesData)) {
          setModules(modulesData);
        }

        const challengeRes = await fetch(`/api/labs/challenges/${id}`);
        const challengeData = await challengeRes.json();
        if (challengeData._id) {
          setTitle(challengeData.title);
          setDescription(challengeData.description);
          setModuleId(challengeData.moduleId);
          setLanguage(challengeData.language);
          setDifficulty(challengeData.difficulty);
          setXp(challengeData.xp);
          setStarterCode(challengeData.starterCode);
          setSolution(challengeData.solution);
          setPublished(challengeData.published);
          setHints(challengeData.hints && challengeData.hints.length > 0 ? challengeData.hints : ['']);
          setValidationRules(
            challengeData.validationRules && challengeData.validationRules.length > 0
              ? challengeData.validationRules
        ...
        console.error(err);
        setError(err.message || 'An error occurred during loading.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="mx-auto max-w-4xl w-full px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="border-b border-border/40 pb-4">
        <h1 className="text-3xl font-extrabold tracking-tight">Edit Challenge</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Modify description, update starter templates, add progressive hints, or rewrite testing scripts.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 text-sm font-bold transition-all disabled:opacity-50 shadow-sm"
          >
